        ...
        if ($table == 'fe_groups') {
            ...
            if (!empty($subgroups)) {
                ...
                foreach ($subgroups as $subgroup) {
                    $usergroupInList[] = 'FIND_IN_SET(' . (int)$subgroup . ', fe_users.usergroup)';
                }
                $usergroupInList = '(' . implode(' OR ', $usergroupInList) . ')';
    ...
    public function getFEgroupSubgroups($groupIds)
    {
        ...
        foreach ($groupIds as $groupId) {
            ...
            $res = $queryBuilder->selectLiteral('DISTINCT fe_groups.uid')
                ->from($table)
                ->where(
                    $queryBuilder->expr()->inSet('fe_groups.subgroup', $queryBuilder->createNamedParameter($groupId))
                )
                ->execute();
