...
class StatisticsController extends MainController
{
    ...
    protected function stats($row)
    {
        ...
        if (intval($row['sendOptions']) & 0x2) {
            ...
            $htmlLinks = [];
            if (isset($unpackedMail['html']['hrefs']) && is_array($unpackedMail['html']['hrefs'])) {
        ...
        if ($urlCounter['total']) {
             ...
             */
            if (isset($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['direct_mail']['mod4']['cmd_stats_linkResponses'])
                && is_array($GLOBALS['TYPO3_CONF_VARS']['EXTCONF']['direct_mail']['mod4']['cmd_stats_linkResponses']))
            {
                $hookObjectsArr = [];
            ...
            foreach($rows as $row) {
                ...
                switch ($row['response_type']) {
                    ...
                    case '2':
                        ...
                        if (!isset($recRec['firstlink'])) {
                            $recRec['firstlink'] = $row['url_id'];
                            if(!empty($recRec['pings'])) {
                                ...
                                $recRec['firstlink_time'] = 0;
                            }
                        } elseif (!isset($recRec['secondlink'])) {
                            ...
                            if(!empty($recRec['pings'])) {
                                ...
                                $recRec['secondlink_time'] = 0;
                            }
                        } elseif (!isset($recRec['thirdlink'])) {
...
//                 }
    ...
    public function getLinkLabel($url, $urlStr, $forceFetch = false, $linkedWord = '')
    {
        ...
        if (isset($this->params['showContentTitle']) && ($this->params['showContentTitle'] == 1)) {
            $label = $contentTitle;
        }

        if (isset($this->params['prependContentTitle']) && ($this->params['prependContentTitle'] == 1)) {
